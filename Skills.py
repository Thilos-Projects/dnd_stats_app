...
def _bonus(values: dict[str, int | float] | None, fields: tuple[str, ...], name: str) -> dict[str, int | float]:
    if values is None:
        return {field: 0 for field in fields}
    if set(values) != set(fields):
        raise ValueError(f"{name} must contain exactly: {', '.join(fields)}")
    for field, value in values.items():
        if isinstance(value, bool) or not isinstance(value, int | float) or not -2 <= value <= 2:
            raise ValueError(f"{name} {field} must be a number between -2 and 2")
    return dict(values)
...
def createSkill(
    ...
    talent_bonus: dict[str, int | float] | None = None,
    skill_bonus: dict[str, int | float] | None = None,
    alters_anstieg: int | float = 0,
) -> str:
    ...
    if isinstance(alters_anstieg, bool) or not isinstance(alters_anstieg, int | float) or alters_anstieg < 0:
        raise ValueError("alters_anstieg must be a non-negative number of years")
